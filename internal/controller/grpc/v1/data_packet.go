...
func (r *V1) ListPacketsByPeriod(ctx context.Context, v1Request *v1.ListPacketsByPeriodRequest) (*v1.ListPacketsByPeriodResponse, error) {
	q := query.FindDataPacketByPeriodQuery{
		Start: v1Request.Start.AsTime().UTC().Format(time.RFC3339),
		End:   v1Request.End.AsTime().UTC().Format(time.RFC3339),
	}

	packets, err := r.u.Queries.FindDataPacketByPeriod.Handle(ctx, q)
